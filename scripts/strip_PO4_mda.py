...
def _strip_trajectory(sim: str) -> None:
    ...
    trjconv_cmd = f"echo '1' | {util.gmxls_bin} trjconv -f {source_dir}/production.gro -o po4_only.gro -n po4_membrane.ndx -s analysis.tpr"
    subprocess.run(trjconv_cmd, shell=True, check=True)

    trjconv_cmd = f"echo '0 1' | {util.gmxls_bin} trjconv -f {source_dir}/production_all.trr -center -o po4_all.xtc -n {staging_dir}/po4_membrane.ndx -s {staging_dir}/analysis.tpr"
    # print(trjconv_cmd)
    subprocess.run(trjconv_cmd, shell=True, check=True)

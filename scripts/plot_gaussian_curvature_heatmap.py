...
# %%
show_figs = False
curr_fig_path = Path("Figures/gauss_curvature_spatial_autocorrelation")
...
for sim in util.simulations:
    ...
    gauss_curvatures = np.zeros_like(h)
    for i in range(h.shape[0]):
        gauss_curvatures[i] = util.gaussian_curvature(h[i], mc.x_step)

    gauss_curvatures -= np.mean(gauss_curvatures)

    acr = fftAutocorrelation(gauss_curvatures)
    ...
    for i in tqdm(range(1, 100)):
        acr.append(
            ...
            / gauss_curvatures.shape[1] ** 2
            / np.std(gauss_curvatures[-i]) ** 2
        )
...
# %%
show_figs = False
curr_fig_path = Path("Figures/gauss_geometric_localization")
curr_fig_path.mkdir(parents=True, exist_ok=True)
...
for sim in np.concatenate((util.simulations, ["1_vbt"])):
    ...
    gauss_curvature_averaged = np.mean(
        gauss_curvatures[-frames_to_average * 3 : -frames_to_average * 2], axis=0
    )
    ...
    for style, style_ext in plot_styles:
        with plt.style.context(style):
            ...
            ax[0].set_title(r"Gaussian Curvature (nm$^{-2}$)")
...
# %%
show_figs = False
curr_fig_path = Path("Figures/gauss_lipid_pearsons")
curr_fig_path.mkdir(parents=True, exist_ok=True)
...
for sim in np.concatenate((util.simulations, ["1_vbt"])):
    ...
    binned_gauss_curvatures = np.zeros(
        (sets_to_consider, gauss_curvatures.shape[1], gauss_curvatures.shape[2])
    )
    ...
    for k, query in queries.items():
        ...
        for i in range(sets_to_consider):
            if i == 0:
                binned_gauss_curvatures[i] = np.mean(
                    gauss_curvatures[-frames_to_average:],
                    axis=0,
                )
            else:
                binned_gauss_curvatures[i] = np.mean(
        ...
        binned_gauss_curvatures -= np.mean(binned_gauss_curvatures)
        ...
        for i in tqdm(range(sets_to_consider)):
            acr.append(
                signal.correlate2d(
                    binned_gauss_curvatures[i],
                    binned_lipid_density[i],
                    boundary="wrap",
                )
                / binned_gauss_curvatures.shape[1] ** 2
                / (np.std(binned_gauss_curvatures[i]) * np.std(binned_lipid_density[i]))
            )
...
# %%
# show_figs = False
# curr_fig_path = Path("Figures/gauss_lipid_pearsons_scatter")
# curr_fig_path.mkdir(parents=True, exist_ok=True)
...
#             ax1.plot(
#                 binned_gauss_curvatures[50].ravel(),
#                 np.roll(binned_lipid_density[50], (direction, 0), axis=(1, 0)).ravel(),
#                 ".",
#             )

#             ax2.imshow(binned_gauss_curvatures[50])
...
# %%
show_figs = False
curr_fig_path = Path("Figures/gauss_cross-leaflet-spatial-corr")
curr_fig_path.mkdir(parents=True, exist_ok=True)
    ...
    for k, query in queries.items():
        ...
        for i in tqdm(range(sets_to_consider)):
            acr.append(
                ...
                / (np.std(binned_lipid_density_upper[i]) * np.std(binned_lipid_density_lower[i]))
            )
